    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100vh", background:t.bg, color:t.text, fontFamily:"'Segoe UI',system-ui,sans-serif", userSelect:"none", overflow:"hidden", border:`1px solid ${t.border}`, borderRadius: isMax ? 0 : 8 }}>
        ...
        <div style={{ display:"flex" }}>
          {[
            ...
            <button key={i} onClick={b.fn}
              style={{ width:46, height:32, display:"flex", alignItems:"center", justifyContent:"center", background:"transparent", border:"none", color:t.muted, cursor:"pointer" }}
      ...
      <div style={{ display:"flex", alignItems:"center", background:t.bg, borderBottom:`1px solid ${t.border}`, height:36, paddingLeft:8, gap:2, flexShrink:0, overflowX:"auto" }}>
        ...
        <button onClick={addTab}
          style={{ width:28, height:28, borderRadius:6, background:"transparent", border:`1px solid ${t.border}`, color:t.muted, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
          <Plus size={12}/>
        </button>
      </div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:4, background:t.panel, borderBottom:`1px solid ${t.border}`, height:40, padding:"0 8px", flexShrink:0 }}>
        ...
        <button onClick={addBookmark} title="Bookmark"
        ...
        <button onClick={()=>{setShowPasswords(p=>!p);setShowSettings(false)}}
          style={{ width:28, height:28, borderRadius:6, background:showPasswords?t.accent:"transparent", border:"none", color:showPasswords?"#fff":t.muted, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <Key size={12}/>
        </button>
        <button onClick={()=>{setShowSettings(s=>!s);setShowPasswords(false)}}
          style={{ width:28, height:28, borderRadius:6, background:showSettings?t.accent:"transparent", border:"none", color:showSettings?"#fff":t.muted, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <Settings size={12}/>
        </button>
      ...
      {showSavePrompt && (
        <div style={{ background:t.accent+"18", borderLeft:`3px solid ${t.accent}`, padding:"7px 12px", display:"flex", alignItems:"center", gap:10, flexShrink:0 }}>
          ...
          <button onClick={savePassword}
            style={{ background:t.accent, border:"none", borderRadius:4, padding:"4px 12px", color:"#fff", fontSize:11, cursor:"pointer" }}>Save</button>
          <button onClick={()=>setShowSavePrompt(false)}
            style={{ background:"transparent", border:"none", color:t.muted, cursor:"pointer" }}><X size={11}/></button>
        </div>
      )}
      ...
      <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
        ...
        {showSettings && (
          <div style={{ width:270, background:t.panel, borderLeft:`1px solid ${t.border}`, display:"flex", flexDirection:"column", flexShrink:0 }}>
            ...
            <div style={{ flex:1, overflow:"auto", padding:14 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                <button onClick={()=>setZoom(z=>Math.max(25,z-25))}
                  style={{ width:28, height:28, borderRadius:6, background:t.bg, border:`1px solid ${t.border}`, color:t.text, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  <ZoomOut size={12}/>
                </button>
                <input type="range" min={25} max={500} step={25} value={zoom}
                  onChange={e=>setZoom(Number(e.target.value))} style={{ flex:1 }}/>
                <button onClick={()=>setZoom(z=>Math.min(500,z+25))}
                  style={{ width:28, height:28, borderRadius:6, background:t.bg, border:`1px solid ${t.border}`, color:t.text, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  <ZoomIn size={12}/>
                </button>
              </div>
              ...
              <div style={{ background:t.bg, border:`1px solid ${t.border}`, borderRadius:8, padding:12, marginBottom:16 }}>
                {[
                  ...
                  { label:"Local-only Storage",  sub:"Credentials never leave device" },
                ].map((row,i)=>(
                ...
                {bookmarks.length === 0
                  ...
                  : bookmarks.map((b,i)=>(
                    <div key={i} style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 10px", borderBottom:i<bookmarks.length-1?`1px solid ${t.border}`:"none" }}>
                      ...
                      <button onClick={()=>{navigate(b.url);setShowSettings(false)}}
                        style={{ background:t.accent, border:"none", borderRadius:4, padding:"2px 8px", color:"#fff", fontSize:10, cursor:"pointer" }}>Go</button>
                      <button onClick={()=>setBookmarks(prev=>prev.filter((_,j)=>j!==i))}
                        style={{ background:"transparent", border:"none", color:"#ef4444", cursor:"pointer", padding:0 }}><X size={10}/></button>
                    </div>
                  ))
                }
              </div>
            ...
            <div style={{ padding:"9px 14px", borderTop:`1px solid ${t.border}`, fontSize:11, color:t.muted }}>
